...
export class MemorySystem {
    ...
    async store(args) {
        ...
        await this.db.run('INSERT INTO memories (content, type, tags, workflow_id, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?)', [args.content, args.type, JSON.stringify(args.tags), args.workflow_id,
    ...
    async retrieve(args) {
        ...
        const results = await this.db.all('SELECT * FROM memories WHERE content LIKE ? LIMIT ?', [`%${args.query}%`, args.limit || 10]);
        return {
            content: [{
                    type: 'text',
