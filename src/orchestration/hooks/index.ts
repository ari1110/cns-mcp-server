...
export class HookHandlers {
   ...
   */
  async handlePreToolUse(args: PreToolUseArgs) {
   ...
   */
  async handleSessionStart(args: SessionStartArgs) {
  ...
  // Helper methods
  private deriveAssociateType(managerType: string): string {
  ...
  private deriveManagerType(associateType: string): string {
